		var baseUrl="http://168.160.111.26";  			//服务器ip
        var chunkSize=5 * 1024  * 1024 ;        		//片数大小 
        var FILE=null;									
        var hasUploaded=0;
        var chunks=0;  
        ...
        //2.校验文件的MD5
        function checkFileMD5(fileName, fileMd5Value, cgi) {
            return new Promise(function(resolve, reject){              
                /*
                ...
                */
				var url = baseUrl + '/' + cgi + '?filename=' + fileName + "&fileMd5Value=" + fileMd5Value + "&method=" + "check";
...
/*
        // 3.上传chunk
        上传文件片，如果文件块已经存在，就不会上传
        fileMd5Value  整个文件的md5值
        chunkList     已存在的文件片(json格式)
        file		  文件片
        cgi			  需要调用的cgi程序名
        fileSize      文件片大小
        
      async function checkAndUploadChunk(fileMd5Value, chunkList, file, cgi, fileSize) {      	
            ...
            {               	    	                   
              ...
              // 如果已经存在, 则不用再上传当前块
               if (!exit)  
               {                    		                              		
                    var index = await upload(i, fileMd5Value, chunks, file, cgi);
                    hasUploaded++;
															
                    var radio = Math.floor((hasUploaded / chunks) * 100);                   
                }
            }
        };
...
*/
		...
		// 3.上传chunk
		var checkAndUploadChunk = (() => {
		  var _ref = _asyncToGenerator(function* (fileMd5Value, chunkList, file, cgi, fileSize) {
		    ...
		    for (var i = 0; i < chunks; i++) {
		      ...
		      if (!exit) {
		        var index = yield upload(i, fileMd5Value, chunks, file, cgi);
		        hasUploaded++;
		  ...
		  return function checkAndUploadChunk(_x, _x2, _x3, _x4, _x5) {
        ...
        */
        function upload(i, fileMd5Value, chunks, file, cgi) {
            return new Promise(function(resolve, reject){
                ...
                $.ajax({
                   url: baseUrl + '/' + cgi + '?method=' + "upload" + "&filename=" + file.name + "&fileMd5Value=" + fileMd5Value, //"upload",  
                 	//url: baseUrl + '/echo.cgi',                  
                    type: "POST",
                    data: form, 				//刚刚构建的form数据对象
                    async: true, 				//异步
                    processData: false, 		//很重要，告诉jquery不要对form进行处理
                    contentType: false, 		//很重要，指定为false才能形成正确的Content-Type
                    success: function (data) {  //获取上传文件信息
                        resolve(data.desc);                        
                    }
                })				
            })
        ...
        // 4. 通知服务器所有分片已上传完成
        function notifyServer(fileMd5Value, file, chunks, cgi) {
			...
			/*
				...
				method			  请求的方法名
			*/
            var url = baseUrl + '/' + cgi +  '?fileMd5Value=' + fileMd5Value + "&filename=" + file.name + "&size=" + file.size  + "&method=" + "merge" + "&chunks=" + chunks;
            $.getJSON(url, function (data) { 
            	alert('上传成功');           	
...
/*
		...
		// 0.响应点击
		async function responseChange(file, url, fileSize) {
    	
        // 第一步：按照 修改时间+文件名称+最后修改时间-->MD5
        FILE = file;  
                   
        // 开始校验
        var fileMd5Value = await md5File(file); // 生成
        			
        // 第二步：校验文件的MD5
        var result = await checkFileMD5(file.name, fileMd5Value, url);		
        ...
        {  
        	if((RESULT.chunkList!= null) && (RESULT.chunkList!= undefined) &&(RESULT.chunkList != ""))
        	{
        		//exit = true;                           
           	 	return ;
        	}            	
        ...
        // 第三步：检查并上传MD5          
       await checkAndUploadChunk(fileMd5Value, RESULT.chunkList, file, url, fileSize);
                 
       await notifyServer(fileMd5Value, file, chunks, url);	          		     	           
    };
...
*/
		...
		var responseChange = (() => {
           var _ref = _asyncToGenerator(function* (file, url, fileSize) {

              FILE = file;

              var fileMd5Value = yield md5File(file); 

              var result = yield checkFileMD5(file.name, fileMd5Value, url);
              ...
              yield checkAndUploadChunk(fileMd5Value, RESULT.chunkList, file, url, fileSize);

              yield notifyServer(fileMd5Value, file, chunks, url);
           });
           return function responseChange(_x, _x2, _x3) {
                  	return _ref.apply(this, arguments);
           };
		})();
...
var ZYFILE = {
    ...
    funDragHover: function(e) {  //文件拖放
        e.stopPropagation();
        e.preventDefault();
        this[e.type === "dragover" ? "onDragOver": "onDragLeave"].call(e.target);
        return this
    },
    funGetFiles: function(e) {   //获取文件
        var self = this;
        this.funDragHover(e);
    ...
    funDealtFiles: function() {  // 处理过滤后的文件，给每个文件设置下标
        var self = this;
    ...
    funDeleteFile: function(delFileIndex, isCb) { // 处理需要删除的文件  isCb代表是否回调onDelete方法  
    ...
    funUploadFiles: function() {   		//上传多个文件
        var self = this;
      ...
      {     	
      	...
      	//上传一个文件后在调用函数上传另一个文件
      	//alert("this.uploadFile[i].name = " + this.uploadFile[i].name);
      }
    ...
    funUploadFile: function(file) {    //上传单个文件接口
    	...
    	*/
        ...
        //这里传递是服务器url 判断这个url是否是指定的，如果是就调用指定函数
        //拿到前端给的url，将这个url传递各个函数
        //在函数中来调用     xhr.open("POST", self.url, true);和 xhr.send(formdata);     
       
        //fileSize = file.size;	  
        //调用断点续传等一系列函数              
    	responseChange(file, self.url, file.size); 
    ...
    funReturnNeedFiles: function() {   // 返回需要上传的文件
            ...
            this.addEvent = function() {
                ...
                function(e) {
                    ...
                    if (ZYFILE.funReturnNeedFiles().length > 0) {
						var filearray = ZYFILE.funReturnNeedFiles();
						for(var i = 0 ; i< filearray.length; i++)
						{
							//alert("filearray[" + i + "].name =" + filearray[i].name + " filearray[" + i + "].size = " + filearray[0].size);							
						}
                        $("#fileSubmit").click()
                    } else {
                        alert("请先选中文件再点击上传")
