...
#define _USER_ "tbs"  				//数据库用户名
#define _PASSWD_ "tbs"				//数据库密码
#define _DBNAME_ "fileInfo"			//数据库表名
...
{
	char * sqlLange = NULL;
	sqlLange = (char *)malloc(len * 4);
	memset(sqlLange, 0x00, len * 4);
	strncpy(sqlLange,sql, len);
    ...
    //2. real_connect
    mysql = mysql_real_connect(mysql,host,user,psword,dbname,0,NULL,0);	   		    
    if(mysql == NULL)
    {
        exit(1);
    }			    
    ...
    if(mysql_query(mysql,sqlLange) != 0)
    {
    	free(sqlLange);
    	exit(1);
    }
    ...
    for (;;)
    {
		...
		{									
	        ...
	        {	        	
	        	...
	        	mysql((char *)_HOST_, (char *)_USER_, (char *)_PASSWD_, (char *)_DBNAME_, rSql, strlen(rSql),request);	        	    			    		         	        	        		        		          
	        }
	        else
	        {
	            len = 0;
		...
		else if(strcasecmp(method, "wxupload") == 0)
		{
			if (contentLength != NULL)
			{				
				char filepath[256] = "/home/tbs/TBSNative/nginx/html/image/logo";
				char rSql[500]={0};
				time_t timer;     							//time_t就是long int 类型
      			struct tm *tblock;
      			timer = time(NULL);
				...
				wx_recv_save_file(len, user, filepath, filename, &size, request);					
	        	sprintf(rSql,"insert into %s values('%s','%s', '%ld', '%s')", "logoInfo", filename,fileMd5Value,len,asctime(tblock));
	        	mysql((char *)_HOST_, (char *)_USER_, (char *)_PASSWD_, "wxLogoFile", rSql, strlen(rSql),request);							
			}											
		}
